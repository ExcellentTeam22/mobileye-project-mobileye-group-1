...
def get_table() -> object:
    """
    A function that read the table from h5 file.
    :return save_table:pandas table of inttention in image.
    """
...
def crop_images_from_table() -> None:
...
def crop_image(img_path: str, x: int, y: int, zoom: int, index: int, color: str) -> str:
    im = Image.open(C.SRC_PATH+'/'+img_path)
    label_name = img_path.replace("leftImg8bit", "gtFine_color")
    im_label = Image.open(C.FILTER_PATH+'/'+label_name)
    pixel_arr = im.crop((x - C.X_SIZE / zoom, y - C.Y_SIZE / zoom, x + C.X_SIZE / zoom, y + C.Y_SIZE / zoom))
    im = pixel_arr.resize((40, 60))
    num_of_orange_pixel_in_image = check_orange_in_relation_to_picture(im_label, x, y)
    pixel_arr_label = im_label.crop((x - C.X_SIZE / zoom, y - C.Y_SIZE / zoom, x + C.X_SIZE / zoom, y + C.Y_SIZE / zoom))
